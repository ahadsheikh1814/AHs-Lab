    ...
    >
      {MenuItems.map((item, index) => (
        <motion.div
          ...
          className={cn(
            "dark:text-white text-primary cursor-pointer tracking-tighter uppercase flex items-center group",
            isOpen ? "flex" : "hidden"
          )}
        >
          <h1
...
const Nav = ({ isOpen, setOpen }) => {
  return (
    <nav className="flex gap-4 items-center">
      ...
      <div
        className={`dark:bg-neutral-800 bg-neutral-200 rounded-xl p-2 transition-transform ${
          isOpen ? "rotate-180" : ""
        }`}
        onClick={() => setOpen(!isOpen)}
      >
